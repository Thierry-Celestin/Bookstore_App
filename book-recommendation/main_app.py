...
from redisdb.cache import get_cached_book, cache_book  # Import the cache functions
...
def main_app():
    ...
    def recommend_books_based_on_genres_and_ratings(user_id):
        ...
        recommended_books = books_collection.aggregate([ 
            ...
            {"$unwind": {"path": "$ratings", "preserveNullAndEmptyArrays": True}},  # Decompose ratings array
            {"$group": {
                "_id": "$_id",
                "title": {"$first": "$title"},
                "author": {"$first": "$author"},
            ...
            {"$sort": {"avg_rating": -1}},  # Sort by highest average rating
    ...
    if action == "Browse Books":
        ...
        if books:
            ...
            df["_id"] = df["_id"].astype(str)  # Convert MongoDB IDs to strings
